...
export default async function AdminProjectsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <AdminListHeader title="Projects" newHref="/admin/projects/new" />
      <div className="flex flex-col divide-y divide-black/10 rounded-2xl border border-black/10 bg-white">
          ...
          <div key={item.id} className="flex items-center justify-between gap-4 px-5 py-4">
            <div>
              ...
              <p className="text-[13px] text-black/50">
                {item.category} · {item.year} · /{item.slug}
              </p>
            </div>
            <div className="flex items-center gap-3">
              <Link href={`/admin/projects/${item.id}`} className="text-[13px] underline">
                Edit
              </Link>
              <DeleteForm action={deleteProject.bind(null, item.id)}>
        ...
        {(items ?? []).length === 0 && (
          <p className="px-5 py-8 text-center text-black/40">No projects yet.</p>
        )}
      </div>
    </div>
  );
